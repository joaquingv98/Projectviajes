...
export default function DrawAnimation({ participants, onComplete }: DrawAnimationProps) {
  ...
  const renderMatchups = () => {
    ...
    return matchups.map((matchup, index) => (
      <div
        key={index}
        className={`bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 opacity-0 animate-fadeIn ${isMobile ? 'p-3' : 'p-4'}`}
        style={{ animationDelay: `${index * 0.2}s`, animationFillMode: 'forwards' }}
      >
        <div className="flex items-center justify-between">
          <span className={`text-white font-semibold truncate flex-1 ${isMobile ? 'text-sm' : ''}`}>{matchup[0]}</span>
          <span className={`text-blue-300 font-bold flex-shrink-0 ${isMobile ? 'mx-2 text-xs' : 'mx-4'}`}>VS</span>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#001B44] via-[#002855] to-[#003366] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      <div className={`w-full max-w-4xl ${isMobile ? 'py-2' : ''}`}>
        <div className={`text-center ${isMobile ? 'mb-4' : 'mb-12'}`}>
          ...
          <h1 className={`font-bold text-white ${isMobile ? 'text-2xl mb-1' : 'text-5xl mb-3'}`}>
        ...
        <div className={`relative ${isMobile ? 'mb-4' : 'mb-8'}`}>
          <div className={`bg-gradient-to-b from-white/10 to-white/5 rounded-2xl sm:rounded-3xl border-2 sm:border-4 border-white/20 backdrop-blur-sm overflow-hidden ${isMobile ? 'p-4' : 'p-8'}`}>
            {isAnimating && (
