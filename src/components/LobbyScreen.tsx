...
export default function LobbyScreen({ tournament, currentUser, onStart }: LobbyScreenProps) {
  ...
  const allJoined = connectedUsers.length >= totalParticipants;
  ...
  useEffect(() => {
    ...
    channel
      .on('presence', { event: 'sync' }, () => {
        ...
        // Eliminar duplicados (por si alguien tiene varias pestañas)
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#001B44] via-[#002855] to-[#003366] flex items-center justify-center p-8">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          <h1 className="text-5xl font-bold text-white mb-3">Sala de espera</h1>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
          <div className="grid grid-cols-2 gap-3">
            {tournament.participants.map((name) => {
              const hasJoined = connectedUsers.includes(name);
              ...
              return (
                ...
                >
                  ...
                  <span className={`font-medium text-sm truncate ${
                    hasJoined ? (isMe ? 'text-blue-200' : 'text-white') : 'text-white/30'
                  }`}>
                    {name}{isMe ? ' (tú)' : ''}
                  </span>
                </div>
        ...
        {starting ? (
          ...
          <div>
            ...
            >
              ...
              {allJoined ? '¡Comenzar sorteo!' : `Esperando a ${totalParticipants - connectedUsers.length} más...`}
            </button>
            {!allJoined && (
