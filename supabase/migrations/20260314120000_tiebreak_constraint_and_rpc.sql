-- Constraint para fases tiebreak en matches
...
-- RPC para avanzar fase tiebreak (SECURITY DEFINER = bypassa RLS)
...
BEGIN
  ...
  IF v_match.status = 'tiebreak_vote' THEN
    ...
    ELSE
      ...
      IF v_is_even THEN
        UPDATE matches SET player1_name = v_winner_name,
          status = CASE WHEN player2_name IS NOT NULL THEN 'proposing' ELSE status END
        WHERE id = v_next_match.id;
      ELSE
        UPDATE matches SET player2_name = v_winner_name, status = 'proposing'
        WHERE id = v_next_match.id;
      END IF;
    END IF;
    RETURN;
...
GRANT EXECUTE ON FUNCTION advance_tiebreaker_phase(uuid) TO anon, authenticated;
